...
## List ##
List 是一个接口，是 Collection 的子接口。可以将 List 理解成存放对象的集合，只不过元素个数不固定。
...
>如果重写 equals 方法，一定要重写 HashCode 方法。

## TreeSet ##
底层采用 二叉树算法，TreeSet 存储对象时，可以进行排序，但需要指定排序算法，依赖 complareTo 方法。
...
## List 特有的方法 ##
可以通过下标来操作元素（Set不可以）
List 除了继承 Collection 定义的方法之外，还定义了自己特有的方法。
常用方法：

E e get(int index):获取集合中指定下标的元素

E e set(int index,E element):将给定的元素存入到集合中指定位置，并将原位置的元素返回。
...
>**子集的修改会影响父集**
	...
	System.out.println(subList);//输出 [1,7,50,4]